...
- **NAME CORRECTION (post-authoring):** the atmospheric input type originally named `Environment` in this plan was renamed to **`Atmosphere`** during Task 2 — `Environment` clashed with SwiftUI's `@Environment` property wrapper in the same module. Everywhere this plan says the type `Environment` or the `environment:` parameter label (notably Task 2 and Task 7), read `Atmosphere` / `atmosphere:`. The `airDensityKgM3` property is unchanged.
